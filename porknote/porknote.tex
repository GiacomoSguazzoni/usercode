...
On a very complex GEANT4 detector description as the one of the CMS Tracker, the method above described can only be practically implemented by leaving the solid dimensions unchanged and modifying the densities of the materials associated to solids, i.e. $k_i \rho_i \ell_i \equiv (\rho_i + \Delta \rho_i) \ell_i$. Within CMSSW this is done by altering the XML files used to describe the detector geometry. A software tool written on purpose consisting of a perl XML parser has been developed on purpose. The material XML files in input are parsed and then, according to a {\em scenario} configuration, the effective densities of the material under study and of all materials that depend on it (directly and indirectly) are changed; finally the modified XML files are written in output. These modified XML files are fully working in CMSSW and actually represent a CMS Tracker with some of its material component altered with respect to the nominal description.
...
the alteration method can be applied with the k-factor different from unity only
...
The four sets of k-factors $\kvect^{\rm R}_{max}$, $\kvect^{\rm R}_{min}$, $\kvect^{\rm I}_{max}$ and $\kvect^{\rm I}_{min}$ can be easily used to configure the parser and produce as many sets of geometry XML files, each corresponding to a specific geometry scenario altered with respect to the default that can be considered representative of the potential systematic uncertainties related to the description of the Tracker material.
...
Moreover the approach presented is very flexible, well beyond the four proposed alteration scenarios. In fact, the deployed tools can easily traslate into fully working geometry XML files any given alteration found interesting to be studied.
