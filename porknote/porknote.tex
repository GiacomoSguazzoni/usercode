% To replace ``Weight'' with ``Mass'' in Appendix A and B
% sed -i 's/Weight/Mass/g' *tex
% To replace ``Silicon'' with ``SiliconSensitive'' in Appendix B
% sed -i 's/Silicon/SiliconSensitive/g' *cats.tex
...
%   \date{1 Brumaio 2010}
  ...
  \begin{Authlist}
    Ernesto Migliore
     \Instfoot{to}{INFN and University of Torino, Italy}
    Giacomo Sguazzoni
     \Instfoot{fi}{INFN Firenze, Italy}
  \end{Authlist}
...
The description of the CMS Tracker~\cite{cmspaper} geometry
implemented in CMSSW, the CMS software 
framework~\cite{CMSSW}, to simulate and reconstruct the events
includes the accurate modelling of the passive materials~\cite{rr}  through
a set of XML files~\cite{ddd} 
then appropriately interfaced with GEANT4~\cite{g4}, the detector
simulation package used in CMS. 
The Tracker features 1440 pixel and 15 148 silicon strip
modules each consisting at least of a silicon sensor, a supporting mechanical
frame, an on board electronic circuit, and a set of electrical and
optical connections. Modules are then mounted on supporting
mechanical structures, shells in the barrel and disks in the endcaps,
housing also the cooling elements and the main power and control
lines. Finally the Tracker is complemented by services which provide
the power and the cooling to the detector.
Though already simplified, the Tracker geometry implemented in
CMSSW consists of more that 350 000 volumes 95\% of them being passive
volumes. The passive volumes are usually filled by average mixtures of
their components with the exceptions of parts, like aluminium blocks,
which corresponds to localised dense materials and which are described
in detail. Therefore the definition of the passive volumes introduces 
of necessity some approximation in the description of the Tracker.
...
distribution in $|\eta|\le$ 5 and $0\le\phi\le2\pi$. The unperturbed
...
The $\eta$ range considered in the analysis of Section~\ref{sec:maxmin} will be 
$|\eta|\le$3, where the average thicknesses determined with the procedure
described above are  $\TX=1.184\pm0.002$ and $\TI=0.3495\pm0.0006$.

The Tracker geometry used by the detector simulation has been deployed
by using at best the technical informations, engineering drawings, and
direct experience of the people contributing to its construction and
assembly. 
...
This note will discuss a possible way to estimate the uncertainty related to the former of these issues. Nevertheless the conservative approach choosen allows the results to give also reasonable insights on systematic uncertainties related to the latter effect.

The inputs of the study are the mass discrepancies,
$\DM$'s. As it will be discussed in Section~\ref{sec:scenarios}, the
$\DM$'s include either the simulated versus weighted mass 
differences, for the subdetectors for which they are available,  
or the uncertainty assumed on the actual mass of a given
material, based on the construction experience.
Nevertheless, the impact
of $\DM$'s on the total thickness in terms of number of radiation
lengths, $\TX$, or in terms of number of interaction lengths, $\TI$,
does depend on how the $\DM$'s are distributed within the
Tracker. Once this dependence is known, the method proposed
allows to understand downstream effects on $\TX$ or $\TI$. 
...
\begin{equation}
\TX = \sum_i \rho_i \frac{\ell_i}{\Xo_i} \equiv \sum_i \tX_i.
\end{equation}
Similarly the total thickness in units of radiation lengths, $\TI$, is
\begin{equation}
\TI = \sum_i \rho_i \frac{\ell_i}{\lI_i} \equiv \sum_i \tI_i.
\end{equation}
...
\begin{equation}
\TX = \sum_i \rho_i \frac{\ell_i}{\Xo_i} \rightarrow  \TX(\kvect) = \sum_i k_i \rho_i \frac{\ell_i}{\Xo_i},
\end{equation}
and
\begin{equation}
\TI = \sum_i \rho_i \frac{\ell_i}{\lI_i} \rightarrow  \TI(\kvect) = \sum_i k_i \rho_i \frac{\ell_i}{\lI_i}.
\end{equation}
...
On a very complex GEANT4 detector description as the one of the CMS
Tracker, the method above described can only be practically
implemented by leaving the solid dimensions unchanged and modifying
the densities of the materials associated to solids, i.e. $k_i \rho_i
\ell_i \equiv (\rho_i + \Delta \rho_i) \ell_i$. Within CMSSW this is
done by altering the XML files used to describe the detector
geometry. A software tool written consisting of a perl XML parser has
been developed on purpose. The material XML files in input are parsed
and then, according to a {\em scenario} configuration, the effective
densities of the material under study and of all materials that depend
on it (directly and indirectly) are changed; finally the modified XML
files are written in output. These modified XML files are fully
working in CMSSW and actually represent a CMS Tracker with some of its
material components altered with respect to the nominal description. 
...
for the following analysis these basic materials were grouped into a
limited number of {\it categories} defined as a class of homogeneous
and/or similar materials. A category can consist of just one basic material.
The category definitions are given in table~\ref{table:cat}.
In the following, indexes referring to categories will be indicated by
greek letters.
\begin{table}[h]
\begin{center}
\begin{tabular}{clll}
...
6 & C6F14 & C6F14 & C6F14 \\
  \hline 
7 & Silicon sensitive & SiliconSensitive & Silicon \\
  \hline 
...
and illustrated in figure~\ref{fig:summaryMaterialsMass}.
...
For example for the thickness in units of radiation length $\TX$, if $k_\beta$ is the factor different from unity
\begin{equation}
\mathrm{\TX}_{(...,1,{k_\beta},1...)} = \mathrm{\TX} + (k_\beta - 1) t_{{\rm R}\beta},
\end{equation}
from which it is straightforward to derive the respective component $t_{R\beta}$:
\begin{equation}
t_{{\rm R}\beta} = \frac{\mathrm{\TX}_{(...,1,k_\beta,1...)} - \mathrm{\TX}}{k_\beta - 1} .
\end{equation}
The contributions of each of the above categories to the $\TX$ and $\TI$, averaged on the volume $|\eta|\le$3, 
are illustrated in the top and bottom plots of figure~\ref{fig:summaryMaterialsTXTI} respectively.
...
\begin{equation}
\mathrm{\TX} + \Delta \mathrm{\TX} = \sum_\alpha \frac{\ell_\alpha}{X_{0 \alpha}} \left( \rho_\alpha + \Delta \rho_\alpha \right) = \sum_\alpha \tX_\alpha k_\alpha,
\end{equation}
\begin{equation}
\mathrm{\TI} + \Delta \mathrm{\TI} = \sum_\alpha \frac{\ell_\alpha}{\lambda_{{\mathrm I} \alpha}} \left( \rho_\alpha + \Delta \rho_\alpha \right) = \sum_\alpha \tI_\alpha k_\alpha.
\end{equation}
...
\item The overall difference between weighted mass and total simulated mass is kept fixed by imposing
 \begin{equation}
\Delta M = \sum_\alpha \frac{\Delta \rho_\alpha}{\rho_\alpha} m_\alpha.
\end{equation}
The Silicon Strip Tracker mass as estimated during the integration and
installation is 3996$^{+88}_{-132}$ kg. The Pixel detector, installed
in a later time, is not included in this estimation. The corresponding
mass in the simulation is 4037 kg. The resulting relative weighted versus simulated mass difference is $-1^{+2}_{-3}\%$ suggesting an agreement between weighted and simulated masses within the estimation error.

Similar estimation performed over single subdetector indicate possible
localized larger discrepancies. For example, the TIB/TID subdetector
plus some ancillary pieces have been weighted. The result is
450$^{+12}_{-20}$ kg to be compared with 427.0 kg the simulated mass of the corresponding volumes. The resulting weighted versus simulated mass difference is $5^{+3}_{-4}\%$. As far as the TEC subdetector is concerned an uncertainty of about 3\% is claimed by the experts.
...
The density of the sensitive SiliconSensitive volume is kept fixed to avoid affecting the signal generated in the active volumes. Nevertheless it is worth noticing that also silicon detector thicknesses can vary up to $\sim 10\%$.
\end{itemize} 
...
\begin{eqnarray*}
\TX(\kvect^{\rm R}) = \sum_\alpha k_\alpha t_{{\rm R}j} +
\epsilon^{\rm R}_r \left[ \left(\sum_\alpha (k_\alpha-1.05) \rho_\alpha V_\alpha\right)^2  + \sum_\alpha
  g(k_\alpha)^2 \right] \quad \text{for determining $\kvect^{\rm R}_{\rm min}$,}\\
\TX(\kvect^{\rm R}) = -\sum_\alpha k_\alpha t_{{\rm R}j}, +
\epsilon^{\rm R}_r \left[ \left(\sum_\alpha (k_\alpha-1.05) \rho_\alpha V_\alpha\right)^2  + \sum_\alpha
  g(k_\alpha)^2 \right] \quad \text{for determining $\kvect^{\rm R}_{\rm max}$,}\\
\TI(\kvect^{\rm I}) = \sum_\alpha k_\alpha t_{{\rm I}j} +
\epsilon^{\rm I}_r \left[ \left(\sum_\alpha (k_\alpha-1.05) \rho_\alpha V_\alpha\right)^2  + \sum_\alpha
  g(k_\alpha)^2 \right] \quad \text{for determining $\kvect^{\rm I}_{\rm min}$,}\\
\TI(\kvect^{\rm I}) = -\sum_\alpha k_\alpha t_{{\rm I},} +
\epsilon^{\rm I}_r \left[ \left(\sum_\alpha (k_\alpha-1.05) \rho_\alpha V_\alpha\right)^2  + \sum_\alpha
  g(k_\alpha)^2 \right] \quad \text{for determining $\kvect^{\rm I}_{\rm max}$.}
\end{eqnarray*}
...
defined in table~\ref{table:catVariations}, e.g. $k_{\alpha,min}\le  k_\alpha\le
k_{\alpha,max}$ where the inequalities are defined by the function:
\[
g(k_\alpha)=
\begin{cases}
k_\alpha-k_{\alpha,min}&  \text{if $k_\alpha<k_{\alpha,min}$,} \\
k_{\alpha,max}-k_\alpha,&  \text{if $k_\alpha>k_{\alpha,max}$,} \\
0 & \text{otherwise.}
\end{cases}
\]
...
\\ 
  \hline 
CarbonFiber             &  1.138  & 1.150  & 1.150 & 1.007 \\
...
SiliconSemsitive        &  Fixed  & Fixed  & Fixed & Fixed \\
...
and~\ref{fig:lambda0minmax}. It is worth notice how a global mass change of $+5\%$ can traslate into a local fractional change of $\TX$ and $\TI$ in the range $-3\%$ to $+8\%$ and $-3\%$ to $+10\%$, respectively, and a non trivial $\eta$ dependence.
...
\bibitem{rr} R.~Ranieri for the CMS Collaboration, {\em The Simulation of the CMS Silicon Tracker}, CERN-CMS-CR-2008-007 (2008) and proceedings of IEEE Nuclear Science Symposium and Medical Imaging Conference, Honolulu, HI, USA, 2007.
\bibitem{ddd} M. Case, M.~Liendl and F.~van~Lingen, {\em Detector Description Domain Architecture \& Data Model}, CMS NOTE-2001/057, (2001).
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.8\textwidth]{fig/SummaryMaterialsMass.pdf}
\end{center}
\caption{Contribution of the categories listed in table~\ref{table:cat} to the total mass of the Tracker. 
...
\label{fig:summaryMaterialsMass}
\end{figure}

\begin{figure}[h]
\begin{center}
\includegraphics[width=0.8\textwidth]{fig/SummaryMaterialsX0.pdf}
\includegraphics[width=0.8\textwidth]{fig/SummaryMaterialsLambda0.pdf}
\end{center}
\caption{Contribution of the categories listed in table~\ref{table:cat} to the average $\TX$ (top) and average $\TI$ (bottom) in the range $|\eta|\le$3. 
Values on the left vertical axis indicates absolute contributions (black) while relative contributions are indicated on the right (red).}
\label{fig:summaryMaterialsTXTI}
\end{figure}
