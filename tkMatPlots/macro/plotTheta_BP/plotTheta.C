...
void plotTheta(){
  ...
  //  TString dir("allStat_6hit_coarse/");
  //  TString dir("allStat_6hit_coarse_coarse/");
  TString dir("./");

  //Agguanta gli istogrammi
  //
  //Raw
  TFile * rMCF = new TFile(dir+"rMC_Theta.root");
  TH1D * rMCH = (TH1D*) rMCF->Get("rMC_Theta");;
  //
  TFile * rSimF = new TFile(dir+"rSim_Theta.root");
  TH1D * rSimH = (TH1D*) rSimF->Get("rSim_Theta");;
  //
  TFile * rDataF = new TFile(dir+"rData_Theta.root");
  TH1D * rDataH = (TH1D*) rDataF->Get("rData_Theta");;
  //
  TFile * rFakeF = new TFile(dir+"rFake_Theta.root");
  TH1D * rFakeH = (TH1D*) rFakeF->Get("rFake_Theta");;
  //
  //Scaled
  TFile * SimF = new TFile(dir+"Sim_Theta.root");
  TH1D * SimH = (TH1D*) SimF->Get("Sim_Theta");;
  //
  TFile * MCF = new TFile(dir+"MC_Theta.root");
  TH1D * MCH = (TH1D*) MCF->Get("MC_Theta");;
  //
  TFile * MCFsF = new TFile(dir+"MCFakeSub_Theta.root");
  TH1D * MCFsH = (TH1D*) MCFsF->Get("MCFakeSub_Theta");;
  //
  TFile * FakeF = new TFile(dir+"MCFake_Theta.root");
  TH1D * FakeH = (TH1D*) FakeF->Get("MCFake_Theta");;
  //
  TFile * DataF = new TFile(dir+"Data_Theta.root");
  TH1D * DataH = (TH1D*) DataF->Get("Data_Theta");;
  //
  TFile * DataFsF = new TFile(dir+"DataFakeSub_Theta.root");
  TH1D * DataFsH = (TH1D*) DataFsF->Get("DataFakeSub_Theta");;
  //
  //Unfold Stuff
  if ( iUf ) {
  ...
  //Frame1
  Double_t Min = 0.008*rDataH->GetBinContent(rDataH->GetMaximumBin());;
  Double_t Max = 12.*rDataH->GetBinContent(rDataH->GetMaximumBin());
  //  Double_t Min = 0.000000009;
  //  Double_t Max = 0.02;
  TH2F *frame1= new TH2F("frame1",";#theta (rad);Countings/event/dR",1.,rDataH->GetXaxis()->GetBinLowEdge(1),rDataH->GetXaxis()->GetBinUpEdge(rDataH->GetNbinsX()),1.,Min,Max);
  ...
  legend->AddEntry(DataH, "Data 8TeV, "+TSsData, "pl");
  ...
  TH2F *frame2= new TH2F("frame2",";#theta (rad);#propto N_{#gamma} #times P/X_{0}/event",1.,DataH->GetXaxis()->GetBinLowEdge(1),DataH->GetXaxis()->GetBinUpEdge(DataH->GetNbinsX()),1.,Min,Max);
  ...
  //  legend->AddEntry(UfBay1DataFsH, "Data 8TeV, fake sub., unfolded Bayes 0 iterations", "pl");
  //  legend->AddEntry(UfBay2DataFsH, "Data 8TeV, fake sub., unfolded Bayes 1 iterations", "pl");
  legend->AddEntry(UfBay3DataFsH, "Data 8TeV, fake sub., unfolded "+TSsDataFs, "pl");
  //  legend->AddEntry(UfBay4DataFsH, "Data 8TeV, fake sub., unfolded Bayes 5 iterations", "pl");
  ...
  TH2F *frame3= new TH2F("frame3",";#theta (rad);#propto N_{#gamma} #times P/X_{0}/event",1.,DataH->GetXaxis()->GetBinLowEdge(1),DataH->GetXaxis()->GetBinUpEdge(DataH->GetNbinsX()),1.,Min,Max);
  //  TH2F *frame2= new TH2F("frame2",";R (cm);X_{0}^{-1} (arbitrary units)",1.,DataH->GetXaxis()->GetBinLowEdge(1),DataH->GetXaxis()->GetBinUpEdge(DataH->GetNbinsX()),1.,Min,Max);
  ...
  rootils::drawEtaValuesOnThetaPlot(Min, Min+(Max-Min)*0.1, 1.9, 3.);
  ...
  legend = new TLegend(legx1,legy2-yoff,legx2,legy2);
  legend->SetNColumns(2);
  legend->SetFillColor(0);
  legend->SetBorderSize(0.);
  //  legend->AddEntry(SimH, "MC truth", "f");
  //  legend->AddEntry(UfBay1DataFsH, "Data 8TeV, fake sub., unfolded Bayes 0 iterations", "pl");
  //  legend->AddEntry(UfBay2DataFsH, "Data 8TeV, fake sub., unfolded Bayes 1 iterations", "pl");
  legend->AddEntry(DataFsH, "Data 8TeV, fake sub., "+TSsDataFs, "pl");
  //  legend->AddEntry(UfBay4DataFsH, "Data 8TeV, fake sub., unfolded Bayes 5 iterations", "pl");
  //  legend->AddEntry(UfBay1MCFsH, "MC reco, fake sub., unfolded Bayes 0 iterations", "pl");
  //  legend->AddEntry(UfBay2MCFsH, "MC reco, fake sub., unfolded Bayes 1 iterations", "pl");
  ...
  Min = 0.0;
  Max = 1.6*SimH->GetBinContent(SimH->GetMaximumBin());
  TH2F *frame4= new TH2F("frame4",";#theta (rad);<1/X_{0}> (cm^{-1})",1.,DataH->GetXaxis()->GetBinLowEdge(1),DataH->GetXaxis()->GetBinUpEdge(DataH->GetNbinsX()),1.,Min,Max);
  //  TH2F *frame2= new TH2F("frame2",";R (cm);X_{0}^{-1} (arbitrary units)",1.,DataH->GetXaxis()->GetBinLowEdge(1),DataH->GetXaxis()->GetBinUpEdge(DataH->GetNbinsX()),1.,Min,Max);
