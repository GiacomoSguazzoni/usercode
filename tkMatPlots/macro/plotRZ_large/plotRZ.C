...
void writePrel(TH2* frame, const char* suffix){
  ...
  TLatex * prel = new TLatex(x0+par*(x1-x0),y0,"CMS Preliminary 2012");
  prel->SetTextSize(.055);
  prel->SetTextAngle(90);
  prel->Draw();

  par = -0.17;
  TLatex * label = new TLatex(x0,y0+par*(y1-y0),Suff);
...
void Plot2D(TH2D * H, char* label, TString dir){
  ...
  std::cout << hSide << " " << vSide << std::endl;

  Int_t sqX = 1200;
  Int_t sqY = 1000;

  //  TCanvas *xyCan = new TCanvas("xyCan","xyCan",((Int_t)(sqX*asRatio)),sqY);
  TCanvas *xyCan = rootils::makeNiceCanvasByPixMargins(2, 2, H->GetNbinsX(), H->GetNbinsY(), 50, 100, 150, 200);
  ...
  */
  ...
  H->SetTitleOffset(0.41,"y");
  H->SetTitleOffset(0.5,"z");
  H->SetLabelOffset(-0.005,"z");
  ...
  //  rootils::drawEtaValues(H->GetXaxis()->GetXmax(), H->GetYaxis()->GetXmax());
  rootils::drawEtaValues(10.,10., 150., 80.);
...
void plotRZ(){
  ...
  std::cout << MCH->GetEntries() << " " << MCH->GetEffectiveEntries() << " " << MCH->Integral() << std::endl;
  std::cout << DataH->GetEntries() << " " << DataH->GetEffectiveEntries() << " " << DataH->Integral() << std::endl;
  ...
  */
  ...
  /*
  std::cout << MCCoarse->GetEntries() << " " << MCCoarse->GetEffectiveEntries() << std::endl;
  std::cout << DataCoarse->GetEntries() << " " << DataCoarse->GetEffectiveEntries() << std::endl;
  */
